  ...
  protected function processTool(string $name): void {
    ...
    if (isset($tool['ahoy'])) {
      foreach ($tool['ahoy'] as $string) {
        File::replaceContentInFile($this->tmpDir . '/.ahoy.yml', $string, '');
        File::replaceContentCallbackInFile($this->tmpDir . '/.ahoy.yml', function (string $content): string {
          return Yaml::collapseEmptyLinesInLiteralBlocks($content);
        });
      }
    }
